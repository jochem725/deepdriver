...
from deepracer_interfaces_pkg.srv import (
    ActiveStateSrv,
    EnableStateSrv,
    GetCalibrationSrv,
    SetCalibrationSrv,
...
from deepracer_interfaces_pkg.msg import ServoCtrlMsg, SoftwareUpdatePctMsg
from webserver_pkg.webserver import app
from webserver_pkg.utility import DoubleBuffer
from webserver_pkg.constants import (
...
class WebServerNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f"Create vehicle state service client: {VEHICLE_STATE_SERVICE}"
        )
        self.vehicle_state_cli = self.create_client(
            ActiveStateSrv, VEHICLE_STATE_SERVICE, callback_group=vehicle_mode_cb_group
        )
        ...
        self.get_logger().info(
            f"Create enable state service client: {ENABLE_STATE_SERVICE}"
        )
        self.enable_state_cli = self.create_client(
            EnableStateSrv, ENABLE_STATE_SERVICE, callback_group=enable_state_cb_group
        )
        ...
        self.get_cal_cli = self.create_client(
            GetCalibrationSrv,
            GET_CAR_CAL_SERVICE,
        ...
        self.set_cal_cli = self.create_client(
            SetCalibrationSrv,
            SET_CAR_CAL_SERVICE,
        ...
        self.get_logger().info(
            f"Create device info service client: {GET_DEVICE_INFO_SERVICE}"
        )
        self.get_revision_info_cli = self.create_client(
            GetDeviceInfoSrv,
            GET_DEVICE_INFO_SERVICE,
        ...
        self.get_logger().info(
            f"Create battery level service client: {BATTERY_LEVEL_SERVICE}"
        )
        self.battery_level_cli = self.create_client(
            BatteryLevelSrv,
            BATTERY_LEVEL_SERVICE,
            callback_group=get_device_info_cb_group,
        )
        ...
        self.get_logger().info(
            f"Create sensor status service client: {SENSOR_DATA_STATUS_SERVICE}"
        )
        self.sensor_status_cli = self.create_client(
            SensorStatusCheckSrv,
            SENSOR_DATA_STATUS_SERVICE,
            callback_group=get_device_info_cb_group,
        )
        ...
        self.get_logger().info(
            f"Create set led color service client: {SET_CAR_LED_SERVICE}"
        )
        self.set_led_color_cli = self.create_client(
            SetLedCtrlSrv, SET_CAR_LED_SERVICE, callback_group=set_led_color_cb_group
        )
        ...
        self.get_logger().info(
            f"Create get led color service client: {GET_CAR_LED_SERVICE}"
        )
        self.get_led_color_cli = self.create_client(
            GetLedCtrlSrv, GET_CAR_LED_SERVICE, callback_group=get_led_color_cb_group
        )
        ...
        self.get_logger().info(
            f"Create verify model ready service client: {VERIFY_MODEL_READY_SERVICE}"
        )
        self.verify_model_ready_cli = self.create_client(
            VerifyModelReadySrv,
            VERIFY_MODEL_READY_SERVICE,
        ...
        self.get_logger().info(
            f"Create configure LiDAR service client: {CONFIGURE_LIDAR_SERVICE}"
        )
        self.configure_lidar_cli = self.create_client(
            LidarConfigSrv, CONFIGURE_LIDAR_SERVICE, callback_group=model_load_cb_group
        )
        self.wait_for_service_availability(self.configure_lidar_cli)

        self.get_logger().info(
            f"Create model state service client: {MODEL_STATE_SERVICE}"
        )
        self.model_state_cli = self.create_client(
            ModelStateSrv, MODEL_STATE_SERVICE, callback_group=model_load_cb_group
        )
        ...
        self.get_logger().info(
            f"Create is model loading service client: {IS_MODEL_LOADING_SERVICE}"
        )
        self.is_model_loading_cli = self.create_client(
            GetModelLoadingStatusSrv,
            IS_MODEL_LOADING_SERVICE,
        ...
        self.get_logger().info(
            f"Create upload model service client: {CONSOLE_MODEL_ACTION_SERVICE}"
        )
        self.model_action_cli = self.create_client(
            ConsoleModelActionSrv,
            CONSOLE_MODEL_ACTION_SERVICE,
        ...
        self.get_logger().info(
            "Create sw update state check service client: "
            f"{SOFTWARE_UPDATE_CHECK_SERVICE_NAME}"
        )
        self.sw_update_state_cli = self.create_client(
            SoftwareUpdateCheckSrv,
            SOFTWARE_UPDATE_CHECK_SERVICE_NAME,
        ...
        self.get_logger().info(
            f"Create begin sw update service client: {BEGIN_UPDATE_SERVICE}"
        )
        self.begin_sw_update_cli = self.create_client(
            BeginSoftwareUpdateSrv,
            BEGIN_UPDATE_SERVICE,
        ...
        self.get_logger().info(
            "Create sw update status service client: "
            f"{SOFTWARE_UPDATE_CHECK_SERVICE_NAME}"
        )
        self.sw_update_status_cli = self.create_client(
            SoftwareUpdateStateSrv,
            SOFTWARE_UPDATE_STATE_SERVICE,
            callback_group=sw_update_cb_group,
        )
        ...
        self.sw_update_pct_sub = self.create_subscription(
        ...
        self.get_logger().info(
            "Create set throttle service client: " f"{AUTONOMOUS_THROTTLE_SERVICE}"
        )
        self.set_throttle_cli = self.create_client(
            NavThrottleSrv,
            AUTONOMOUS_THROTTLE_SERVICE,
        ...
        # Create a reentrant callback group to call traffic max speed update service.
        set_traffic_max_speed_cb_group = ReentrantCallbackGroup()
        self.get_logger().info(
            "Create traffic set max speed service client: "
            f"{TRAFFIC_SET_MAX_SPEED_SERVICE}"
        )
        self.set_traffic_max_speed_cli = self.create_client(
            SetMaxSpeedSrv,
            TRAFFIC_SET_MAX_SPEED_SERVICE,
            callback_group=set_traffic_max_speed_cb_group,
        )
        self.wait_for_service_availability(self.set_traffic_max_speed_cli)

        # Create a reentrant callback group to call Get Ctrl Modes service.
        get_ctrl_modes_cb_group = ReentrantCallbackGroup()
        self.get_logger().info(
            "Create Get Ctrl Modes service client: " f"{GET_CTRL_MODES_SERVICE}"
        )
        self.get_ctrl_modes_cli = self.create_client(
            GetCtrlModesSrv,
            GET_CTRL_MODES_SERVICE,
        ...
        self.get_logger().info(
            "Create otg link state service client: " f"{GET_OTG_LINK_STATE_SERVICE}"
        )
        self.otg_link_state_cli = self.create_client(
            OTGLinkStateSrv,
            GET_OTG_LINK_STATE_SERVICE,
        ...
        self.pub_calibration_drive = self.create_publisher(
            ServoCtrlMsg, CAL_DRIVE_TOPIC, 1, callback_group=cal_pub_drive_msg_cb_group
        )
        ...
        self.pct_dict_db.put({"status": "unknown", "update_pct": 0.0})
    ...
    def sw_update_pct_sub_cb(self, pct_dict):
        ...
        self.pct_dict_db.put(
            {"status": pct_dict.status, "update_pct": int(pct_dict.update_pct)}
        )
    ...
    def wait_for_service_availability(self, client):
        ...
        while not client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                f"{client.srv_name} service not available, waiting again..."
            )
