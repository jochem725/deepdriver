...
def api_set_drive_mode():
    """API to toggle the drive mode between Autonomous/Manual/Traffic mode.
...
@VEHICLE_CONTROL_BLUEPRINT.route("/api/traffic_max_speed", methods=["PUT", "POST"])
def max_traffic_speed():
    ...
    """
    webserver_node = webserver_publisher_node.get_webserver_node()
    traffic_speed = request.json.get("throttle")
    if traffic_speed is None:
        return jsonify({"success": False, "reason": "value must be set."})
    webserver_node.get_logger().info(f"Setting max traffic speed to {traffic_speed}%")
    try:
        set_speed_req = SetMaxSpeedSrv.Request()
        set_speed_req.max_speed_pct = float(traffic_speed / 100)
        set_speed_res = call_service_sync(webserver_node.set_traffic_max_speed_cli, set_speed_req)
